...
class ModelEmbeddings(nn.Module): 
    ...
    def forward(self, input):
        ...
        (sentence_length, batch_size, max_word_length, e_char) = x_emb.size()
        x_reshaped = (x_emb.view(sentence_length*batch_size, max_word_length, e_char)).permute(0,2,1)
        x_conv_out = self.model_cnn(x_reshaped)
        x_highway = self.model_highway(x_conv_out)
        x_word_emb = self.dropout(x_highway)
        return x_word_emb.view(sentence_length, batch_size, self.embed_size)
        ### END YOUR CODE
