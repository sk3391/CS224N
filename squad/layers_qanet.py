    ...
    def forward(self, x, mask, convmask):
        ...
        for h in range(self.n_heads):
            ...
            for k in range(max(0,h-n), min(h+n+1,self.n_heads)):
                ...
                energy = Variable(torch.bmm(Q, K.permute(0,2,1)))
                #print("Energy k = " + str(energy.size()))
                ###print("Self Attention QK.T = " + str(out.size()))
                energy = Variable(torch.mul(energy, normalize))
                #print("Energy Normalize k = " + str(energy.size()))
                out.append(energy)
            energy = Variable(torch.stack(out).permute(1,0,2,3))
            #print("Stacked Energy = " + str(energy.size()))
            #print("Stacked V = " + str(V.size()))
            energy = Variable(torch.mul(energy, convmask))
            #print("Convolved Energy = " + str(energy.size()))
            (bs, nh, sl, dk) = energy.size()
            energy = Variable(energy.contiguous().view(bs, nh*sl, dk).permute(0,2,1))
            #print("Convolved Energy Ready for Softmax = " + str(energy.size()))
            #print("New Mask Ready for Softmax = " + str(newmask.size()))
            newmask = Variable(newmask.contiguous().view(bs, nh*sl))
            alpha = Variable(masked_softmax(energy, newmask.view(batch_size, 1, energy.size(2)), dim=2))
            #print("Alpha after softmax = " + str(alpha.size()))
            alpha = Variable(alpha.view(bs, sl, nh, dk).permute(0, 2, 1, 3))
            #print("Apha Unconvolved = " + str(alpha.size()))
            y = Variable(torch.bmm(alpha.contiguous().view(bs*nh, sl,dk), V.contiguous().view(bs*nh, V.size(2), V.size(3))))
            y = Variable(y.contiguous().view(bs, nh, sl, V.size(3)).sum(dim = 1))
