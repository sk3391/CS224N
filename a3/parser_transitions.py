...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        batch_size_parses = unfinished_parses[:batch_size]
        transitions = model.predict(batch_size_parses)
        for idx, transition in enumerate(transitions):
            batch_size_parses[idx].parse_step(transition)
            if len(batch_size_parses[idx].buffer) == 0 and len(batch_size_parses[idx].stack) == 1:
                unfinished_parses.remove(batch_size_parses[idx])
