...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        W = nn.Linear(self.hidden_size, self.n_features * self.embed_size)
        self.embed_to_hidden = nn.init.xavier_uniform_(W.weight, gain=1)
        self.dropout = nn.Dropout(p = dropout_prob)
        U = nn.Linear(self.n_classes, self.hidden_size)
        self.hidden_to_logits = nn.init.xavier_uniform_(U.weight, gain=1)
        ### END YOUR CODE
    ...
    def embedding_lookup(self, t):
        ...
        embed_lookup = self.pretrained_embeddings(t)
        batch_size, n_features, embedding_size = embed_lookup.size()
        x = embed_lookup.view(batch_size, n_features * embedding_size)
        ### END YOUR CODE
        return x
    ...
    def forward(self, t):
        ...
        h = nn.functional.relu(torch.mm(embeddings, self.embed_to_hidden))
        dropout = self.dropout(h)
        logits = torch.mm(dropout, self.hidden_to_logits)
        ### END YOUR CODE
        return logits
